...
def cis_eig_get_matrix(A, n_eigs=3, phasing_track=None, ignore_diags=2, clip_percentile=0,
            sort_metric=None):
    """
...
def cooler_cis_eig_get_matrix(
    ...
    mats_per_reg = zip(*map(_each, regions))
    return mats_per_reg
...
def cis_eig_CVX(A, phasing_track=None, ignore_diags=2, clip_percentile=0,
            sort_metric=None):
    """
    Compute compartment eigenvector on a dense cis matrix
    Note that the amplitude of compartment eigenvectors is weighted by their corresponding eigenvalue
    ...
    eigvecs, eigvals = numutils.get_eig_CVX(OE, mask_zero_rows=True)
    eigvecs /= np.sqrt(np.nansum(eigvecs**2, axis=1))[:, None]
    eigvecs *= np.sqrt(np.abs(eigvals))[:, None]
